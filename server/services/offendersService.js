...
module.exports = function createOffendersService(
  ...
  async function getDueRecats(
    ...
    // cannot call with Promise.all above, due to prisoner search API single concurrent request restriction
    const prisonersByBookingIdsResponse = await prisonerSearchClient.getPrisonersByBookingIds(
      allOffenders.map(o => o.bookingId)
    )
    const getKeyForBooking = prisoner => `${prisoner.offenderNo}-${prisoner.bookingId}`
    const prisonersByBookingId = prisonersByBookingIdsResponse.reduce((acc, prisoner) => {
      acc.set(getKeyForBooking(prisoner), prisoner)
      return acc
    }, new Map())
